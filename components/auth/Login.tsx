...
const Login: React.FC<LoginProps> = ({ onSwitchToRegister, onGoHome, theme }) => {
    ...
    <div className={`min-h-screen flex items-center justify-center p-6 transition-colors duration-500 ${isDarkMode ? 'bg-[#121212]' : 'bg-[#F8FAFC]'}`}>
      <div className={`max-w-md w-full p-12 rounded-[48px] border transition-all duration-500 ${isDarkMode ? 'bg-[#1e1e1e] border-[#3a3a3a] shadow-2xl' : 'bg-white border-slate-100 shadow-xl'}`}>
        <div className="flex flex-col items-center text-center mb-12">
          ...
          <h1 className={`text-2xl font-black mb-1 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Welcome Back</h1>
          <p className={`text-[11px] font-bold uppercase tracking-[0.25em] ${isDarkMode ? 'text-[#b0b0b0]' : 'opacity-40'}`}>Sign in to your account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className={`block text-[11px] font-black uppercase tracking-widest mb-3 ${isDarkMode ? 'text-[#b0b0b0]' : 'opacity-50'}`}>
              Email Address
            </label>
            <input
              type="email"
        ...
        <div className={`mt-6 p-4 rounded-2xl text-xs ${isDarkMode ? 'bg-[#252525] border border-[#3a3a3a]' : 'bg-slate-50 border border-slate-200'}`}>
          <p className={`font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-slate-700'}`}>Demo Mode</p>
          <p className={isDarkMode ? 'text-[#b0b0b0]' : 'opacity-60'}>
            Register a new account to explore all features. Your progress will be saved locally.
          </p>
        </div>
      </div>
    </div>
